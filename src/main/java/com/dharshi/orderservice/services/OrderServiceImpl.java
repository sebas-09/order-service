...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public ResponseEntity<ApiResponseDto<?>> cancelOrder(String orderId)
    ...
    private boolean sendConfirmationEmail(UserDto user, Order order) {
        StringBuilder contentBuilder = new StringBuilder("Estimado " + user.getUsername() + ",<br><br>"
                ...
                + "<p>Tu Orden #" + order.getId() + " ¡Se ha colocado correctamente!</p>"
        ...
        content += "<h4>Total: " + order.getOrderAmt() + "</h4>"
                + "<p>¡Los gastos de envío se añadirán al total en su puerta!</p>"
                + "<br>Muchas Gracias,<br>"
                + "EcoVida.";

        MailRequestDto mail = MailRequestDto.builder()
                .to(user.getEmail())
                .subject("EcoVida - Confirmacion Orden")
                .body(content)
                .build();
